...
public sealed class JamWeaverConsole
{
    private readonly TextReader reader;
    private readonly ConsoleDisplay display;
    private readonly SafeMidiOutput output;
    private readonly TransportEngine transport;
    private readonly InternalMidiClock internalClock;
    private readonly PatternPlayer player;
    private readonly CandidateSession session;
    private readonly CandidateGenerator candidateGenerator;
    private readonly GenerationControls generationControls;
    private readonly PatternMutator mutator;
    private readonly PhrasePatternMutator phraseMutator;
    private readonly CandidateHistory history;
    private readonly PatternLibrary patternLibrary;
    private readonly MelodicPhraseGenerator phraseGenerator;
    private readonly MelodicGrooveGenerator grooveGenerator;
    private readonly IRandomSource seedSource;
    private readonly ExternalMidiClockInput externalClock;
    private Pattern? comparisonPhrase;
    private Pattern? comparisonGroove;
    private bool comparisonShowsGroove = true;
    private DryWetMidiInput? input;

    public JamWeaverConsole(TextReader reader, ConsoleDisplay display, SafeMidiOutput output,
        TransportEngine transport, InternalMidiClock internalClock, PatternPlayer player,
        CandidateSession session, CandidateGenerator candidateGenerator, GenerationControls generationControls,
        PatternMutator mutator, PhrasePatternMutator phraseMutator, CandidateHistory history,
        PatternLibrary patternLibrary, MelodicPhraseGenerator phraseGenerator,
        MelodicGrooveGenerator grooveGenerator, IRandomSource seedSource, ExternalMidiClockInput externalClock)
    {
        this.reader = reader ?? throw new ArgumentNullException(nameof(reader));
        this.display = display ?? throw new ArgumentNullException(nameof(display));
        this.output = output ?? throw new ArgumentNullException(nameof(output));
        this.transport = transport ?? throw new ArgumentNullException(nameof(transport));
        this.internalClock = internalClock ?? throw new ArgumentNullException(nameof(internalClock));
        this.player = player ?? throw new ArgumentNullException(nameof(player));
        this.session = session ?? throw new ArgumentNullException(nameof(session));
        this.candidateGenerator = candidateGenerator ?? throw new ArgumentNullException(nameof(candidateGenerator));
        this.generationControls = generationControls ?? throw new ArgumentNullException(nameof(generationControls));
        this.mutator = mutator ?? throw new ArgumentNullException(nameof(mutator));
        this.phraseMutator = phraseMutator ?? throw new ArgumentNullException(nameof(phraseMutator));
        this.history = history ?? throw new ArgumentNullException(nameof(history));
        this.patternLibrary = patternLibrary ?? throw new ArgumentNullException(nameof(patternLibrary));
        this.phraseGenerator = phraseGenerator ?? throw new ArgumentNullException(nameof(phraseGenerator));
        this.grooveGenerator = grooveGenerator ?? throw new ArgumentNullException(nameof(grooveGenerator));
        this.seedSource = seedSource ?? throw new ArgumentNullException(nameof(seedSource));
        this.externalClock = externalClock ?? throw new ArgumentNullException(nameof(externalClock));
    }

    public async Task RunAsync()
    {
        externalClock.MessageReceived += ProcessExternalMessage;
    ...
    static GrooveSelection ParseGrooveSelection(string value) => value.ToLowerInvariant() switch
    {
        ...
        "offbeat" => GrooveSelection.Offbeat,
        "anticipation" => GrooveSelection.Anticipation,
        "long-short" => GrooveSelection.LongShort,
        "sparse-answer" => GrooveSelection.SparseAnswer,
    ...
    static MotifShape ParseMotifShape(string value) => value.ToLowerInvariant() switch
    {
        ...
        "root-fifth" => MotifShape.RootFifth,
        "walking" => MotifShape.Walking,
        "call-response" => MotifShape.CallResponse,
        "arch" => MotifShape.Arch,
        "pickup" => MotifShape.Pickup,
        "riff" => MotifShape.Riff,
        _ => throw new ArgumentException("Unknown motif shape.")
    };
}
